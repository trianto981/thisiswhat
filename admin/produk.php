<?php 
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			if($q){ alert("Data Berhasil Dihapus"); redir("produk.php"); }
		}  
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			...
			if($q){
				...
				if($upload){ alert("Data Berhasil Dibuat"); redir("produk.php"); }
			}
		}
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){ 
			...
			if($q){
				$upload = move_uploaded_file($_FILES['gambar']['tmp_name'], '../uploads/'.$gambar);
				if($upload){ alert("Data Berhasil Diedit"); redir("produk.php"); }
			}
		}
	}
...
?> 
	...
	<div class="container">
		...
		<a class="btn btn-sm btn-warning" href="produk.php?act=create">Tambah Data</a>
		<hr>
		<?php
			if(!empty($_GET)){
				if($_GET['act'] == 'create'){
		...
		<?php while($data=mysql_fetch_object($q)){ ?> 
				<tr> 
					...
					<td>
						<a class="btn btn-sm btn-info" href="produk.php?act=edit&&id=<?php echo $data->id ?>">Edit</a>
